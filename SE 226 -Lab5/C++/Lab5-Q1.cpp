...
using namespace std;
//Q1
string passGen(string wordsarray[],int numberofwords){
    string pass = "";
    for(int i = 0;i<numberofwords;i++){
    int number = rand() % 1001;
    pass = pass + wordsarray[i];
     }
...
string rep_with_upper(string word){
    int random = rand() % word.length() + 1;
    char randomletter = word[random];
    char neww = toupper(word[random]);
    std::replace(word.begin(), word.end(), randomletter, neww);
    cout<<word<<endl;
    return word;
...
string swap_letters(string word){
    std::swap(word[0], word[word.length() - 2]);
    std::swap(word[1], word[word.length() - 1]);
    cout<<word<<endl;
}

//Q3
bool search_letter(string source,char letter){
...
int main() {
    ...
    string firstpass = passGen(words,user_number);

    //Q2-1
    string nextpass = rep_with_upper(firstpass);

    //Q2-2
    string finalpass = swap_letters(nextpass);

    char firstletterofmyname;
    firstletterofmyname = 'e';


    //Q3
    search_letter(finalpass,firstletterofmyname);
    if(search_letter(finalpass,firstletterofmyname)){
        cout<<"True"<<endl;//not printing idk why  :(
    }
